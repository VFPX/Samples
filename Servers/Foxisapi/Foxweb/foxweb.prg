...
PROCEDURE Hello
	...
	*-- Check for valid text values.
	*-- In order to prevent cross-site scripting problems, parse for 
	*-- possible HTML tags.  Ideally, one should parse for valid characters
	*-- that you would expect, rather than parsing for known scripting
	*-- characters
	IF TYPE("m.p1")#"C" OR EMPTY(m.p1) OR "<" $ m.p1 OR ">" $ m.p1
		m.p1 = "World"
	ENDIF
	IF TYPE("m.p2")#"C"
		m.p2 = ""
